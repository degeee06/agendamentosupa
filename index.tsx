...
const HelpCircleIcon = (props) => <Icon {...props}><circle cx="12" cy="12" r="10"></circle><path d="M9.09 9a3 3 0 0 1 5.83 1c0 2-3 3-3 3"></path><line x1="12" y1="17" x2="12.01" y2="17"></line></Icon>;
...
const Sidebar = () => {
  ...
  return (
    <div className="h-screen w-64 bg-[#111111] text-[#F5F5F7] flex flex-col p-4 border-r border-white/10 fixed">
        ...
        <ul>
          {navItems.map((item, index) => (
            <li key={index} className="mb-2">
              <a href="#" className={`flex items-center gap-3 p-3 rounded-lg transition-colors ${index === 1 ? 'bg-white/10 text-white' : 'hover:bg-white/5 text-gray-400'}`}>
      ...
      <div>
        <a href="#" className="flex items-center gap-3 p-3 rounded-lg hover:bg-white/5 text-gray-400 transition-colors">
...
const AppointmentCard: React.FC<{ appointment: Appointment }> = ({ appointment }) => {
  ...
  return (
    <div className="glassmorphism p-5 rounded-xl hover:border-white/20 transition-all duration-300">
...
const AppointmentsDashboard = ({ appointments, loading, error }) => {
                ...
                <div className="flex items-center justify-between">
                    <div className="flex border-b border-white/10">
                        {tabs.map(tab => (
                            <button
                                ...
                                className={`px-4 py-2 text-sm font-medium transition-colors ${activeTab === tab ? 'text-white border-b-2 border-[#C0C0C0]' : 'text-gray-400 hover:text-white'}`}
                            >
                                {tab}
                            </button>
                        ))}
                    ...
                    <div className="flex items-center gap-4">
                        <div className="relative">
                            ...
                            className="bg-white/5 border border-white/10 rounded-lg py-2 pl-10 pr-4 w-64 text-sm focus:outline-none focus:ring-2 focus:ring-[#C0C0C0]/50" />
                            <SearchIcon className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400" />
                        </div>
                        <button className="bg-white/5 hover:bg-white/10 text-white font-medium py-2 px-3 rounded-lg transition-colors flex items-center gap-2 text-sm">
...
const MainApp = () => {
    ...
    useEffect(() => {
        fetchAppointments(); // Busca inicial

        // Configura o polling para buscar dados a cada 15 segundos
    ...
    const handleSave = async (newAppointmentData) => {
        ...
        if (error) {
            ...
            fetchAppointments(); // Busca os dados novamente para atualização imediata
        }
    };
        ...
        <div className="flex min-h-screen bg-black">
          ...
          <main className="ml-64 flex-1 flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto scrollbar-hide">
                <AppointmentsDashboard appointments={appointments} loading={loading} error={error} />
            </div>
             <AppointmentModal 
                isOpen={isModalOpen} 
