            ...
            {step === 'info' && (
                <div className="text-center">
                    ...
                    <div className="bg-gray-800 p-4 rounded-lg mb-6">
                        ...
                        <p className="text-3xl font-bold text-white">R$ 00,01<span className="text-sm font-normal text-gray-500">/mês</span></p>
                    </div>
                    <button 
                        onClick={() => setStep('cpf')}
...
const PaginaDeAgendamento = ({ tokenId }: { tokenId: string }) => {
        ...
        if (paymentData?.id && pendingAppointmentId && !bookingCompleted) {
            const checkStatus = async () => {
                try {
                    ...
                    // Agora usando stripe-webhook como proxy para Asaas
        ...
        try {
            const { data, error = null } = await supabase.functions.invoke('create-payment', {
                body: {
                    ...
                    payerCpf: cpf.replace(/\D/g, '') // Remove formatação do CPF para Asaas
                }
            });
            ...
            setMessage({ type: 'error', text: "Erro ao gerar o pagamento Pix: " + err.message });
        }
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        // Validação simples de CPF (presença) se houver preço
        if (businessProfile?.service_price && businessProfile.service_price > 0) {
            ...
            if (unmaskedCpf.length !== 11) {
                setMessage({ type: 'error', text: 'Por favor, insira um CPF válido para gerar o pagamento.' });
                return;
            }
        }
                            ...
                            <form onSubmit={handleSubmit} className="space-y-6">
                                ...
                                {businessProfile?.service_price && businessProfile.service_price > 0 && (
                                    <input 
                                        ...
                                        required 
                                        maxLength={14} 
                                        className="w-full bg-black/20 border border-gray-600 rounded-lg p-3 text-white" 
                                    />
                                )}
    ...
    const handleLogin = async () => {
        ...
        try {
            ...
            if (isNative) {
                ...
                const { data, error = null } = await (supabase.auth as any).signInWithOAuth({
                    provider: 'google',
                    options: {
                        redirectTo,
                ...
                const { error = null } = await (supabase.auth as any).signInWithOAuth({
                    provider: 'google',
                    options: {
                        redirectTo,
                    },
        ...
        if (platform === 'web' && messaging) {
            ...
            try {
                ...
                if (permission === 'granted') {
                    ...
                    if (token) {
                        console.log("Token Web Push:", token);
    ...
    const handleTestNotification = async () => {
        ...
        try {
            ...
            if (data?.success) {
                ...
                // AGORA TRATAMOS O ERRO DETALHADO RETORNADO PELA EDGE FUNCTION
                alert(`Erro no envio: ${data?.details?.error || JSON.stringify(data?.details)}`);
            }
